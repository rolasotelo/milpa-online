import { score_on_tomatoe_played } from "../..";
import {
...
test("should return new scores which depend on modifiers", () => {
  ...
  const expectedNewScore = [1 + CACAO_WHEN_TOMATOE_PLAYED, 1];
  const newScores = score_on_tomatoe_played(oldScores, []);
  expect(newScores).toEqual(expectedNewScore);
  const newScoresWithModifier = score_on_tomatoe_played(oldScores, [
    ModifierId.Manure,
  ]);
  const expectedNewScoreWithModifier = [
    1 + MANURE_MUTIPLIER * CACAO_WHEN_TOMATOE_PLAYED,
    1,
  ];
